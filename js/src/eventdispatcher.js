 ...
 */
var EventDispatcher = Elf.EventDispatcher = function(){
...
EventDispatcher.prototype.addEventListener = function(type, listener){
    ...
    if(map.indexOf(listener) == -1){
    	map.push(listener);
		Elf.eventManager.bind(type,this);
    }
    return this;
};
...
EventDispatcher.prototype.removeEventListener = function(type, listener){
	...
	for(var i = 0; i < map.length; i++){
		...
		if(li === listener){
			...
			Elf.eventManager.unbind(type,this);
			return true;
		}
	}
	return false;
...
EventDispatcher.prototype.unbind = function(type,listener){
	if(type==null && listener==null){
		...
		this.removeEventListenerByType(type);
	}else{
		this.removeEventListenerByType(type,listener);
	}
};
...
//添加若干的常用的快捷缩写方法
EventDispatcher.prototype.bind = EventDispatcher.prototype.addEventListener;
