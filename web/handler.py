...
class Base(tornado.web.RequestHandler):
    def rf(self,key,defaults=''):
        return self.get_argument(key,defaults)
...
class QR(Base):
    ...
    def handle_parameter(self, chl, chld, chs):
        ...
        if len(chl) > 2953: # 最大容量
            # chl = chl[:2953]
            raise HTTPError(200, "chl 超过最大容量 2953字")
            # chl = ''
        ...
        try:
            ...
            raise HTTPError(200, "chs参数不符合300x300")
        else:
            # 处理负数及零的情况
            # 限制图片大小，防止图片太大导致系统死机
        ...
        if level == 'L':
            for i in l_max:
                ...
                raise HTTPError(200, "超出了该纠错级别所能处理的最大字符数")
    ...
    def get(self):
        """处理 POST 数据
        """
        self.redirect("http://chart.apis.google.com/chart?cht=qr&chs=230x230&choe=UTF-8&chld=L&chl=dcgdfgdfg")
        self.finish()
        return 
